...
class CommonHelperMethods:
    ...
    @staticmethod
    def check_commander_version():
        try:
            release_details = CommonHelperMethods.get_latest_release_details()
            release_download_url = release_details.get('release_url')
            curr_app_version = __version__
            curr_git_version = release_details.get('tag')[1:]

            version_comparison = CommonHelperMethods.__version_compare(curr_app_version, curr_git_version)

            if version_comparison < 0:

                logging.warning((bcolors.WARNING +
                                 "---------------------------------------------------------------------------\n"
                                 "| You are running older version of Keeper Commander CLI tool.\n"
                                 "| Current version is %s can be downloaded from %s \n"
                                 "| or follow instruction on how to upgrade it here: \n"
                                 "| https://github.com/Keeper-Security/Commander#upgrading-to-latest-python-code"
                                 + "\n ---------------------------------------------------------------------------\n"
                                 + bcolors.ENDC)
                                % (curr_git_version, release_download_url))
        except Exception:
    ...
    def get_latest_release_details():
        ...
        rs = requests.get(repo_latest_release_api_url)
