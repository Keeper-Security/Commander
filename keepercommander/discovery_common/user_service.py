...
class UserService:
                ...
                for user_vertex in resource_vertex.has_vertices():
                    ...
                    if acl_edge is not None:
                        ...
                        if service_acl.is_used:
                            ...
                            user_acl.rotation_settings.controls_services = True
    ...
    def get_resource_vertices(self, user_uid: str) -> List[DAGVertex]:
        ...
        for resource_vertex in user_vertex.belongs_to_vertices():
            ...
            if user_acl:
                if user_acl.rotation_settings.controls_services:
    ...
    def to_dot(self, graph_format: str = "svg", show_version: bool = True, graph_type: str = "dot"):
        ...
        for resource_vertex in self.record_linking.dag.get_root.has_vertices():
            if resource_vertex.active is False:
                continue

            plotted_resource = False
            for user_vertex in resource_vertex.has_vertices():
                acl = self.record_linking.get_acl(user_vertex.uid, resource_vertex.uid)
                if acl is None or not acl.rotation_settings.controls_services:
                    continue
    ...
    def _init_cleanup_user_mapping(self):
        ...
        for resource_vertex in self.record_linking.dag.get_root.has_vertices():
            ...
            for user_vertex in resource_vertex.has_vertices():
                acl = self.record_linking.get_acl(user_vertex.uid, resource_vertex.uid)
                if acl is not None and acl.rotation_settings.controls_services:
