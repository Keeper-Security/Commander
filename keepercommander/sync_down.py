...
def sync_down(params, record_types=False):   # type: (KeeperParams, bool) -> None
    ...
    while not done:
        ...
        if len(response.removedRecordLinks) > 0:
            for rrl in response.removedRecordLinks:
                child_uid = utils.base64_url_encode(rrl.childRecordUid)
                if child_uid in params.record_link_cache:
                    parents = params.record_link_cache[child_uid]
                    parent_uid = utils.base64_url_encode(rrl.parentRecordUid)
