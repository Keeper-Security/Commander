...
class DAG:
    ...
    def close(self):
        try:
            ...
            if hasattr(self, '_vertices') and hasattr(self, 'uid') and hasattr(self, '_uid_lookup'):
                if len(self._vertices) > 0 and self.uid is not None and self.uid in self._uid_lookup:
    ...
    def debug_stacktrace(self):
        ...
        if exc is not None:
            msg += '  ' + traceback.format_exc().removeprefix(trc)
        self.debug(msg)
    ...
    def get_vertices_by_path_value(self, path: str, inc_deleted: bool = False) -> List[DAGVertex]:
        ...
        for vertex in vertices:
            for edge in vertex.edges:
                if edge is not None and edge.path == path:
