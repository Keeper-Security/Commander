...
class BreachWatch(object):
        ...
        if record.password:
            ...
            if bw_record:
                ...
                if data_obj and 'passwords' in data_obj:
                    password = next((x for x in data_obj['passwords'] if x['value'] == record.password), None)
                    if password:
                        return
    ...
    def get_record_status(params, record_uid):  # type: (KeeperParams, str) -> Optional[dict]
        ...
        if bw_record:
            ...
            if data_obj and 'passwords' in data_obj:
                record = api.get_record(params, record_uid)
                return next((x for x in data_obj['passwords'] if x['value'] == record.password), None)
    ...
    def get_records(params, callback, owned=False):
        ...
        for record_uid in params.record_cache:
            ...
            if params.breach_watch_records:
                ...
                if data_obj and 'passwords' in data_obj:
                    password_dict = next((x for x in data_obj['passwords'] if x['value'] == record.password), None)
