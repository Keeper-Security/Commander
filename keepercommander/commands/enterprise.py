...
security_audit_report_parser.add_argument('--format', dest='format', action='store', choices=['csv', 'json', 'table'], default='csv', help='output format.')
                ...
                if 'users' in params.enterprise:
                    for user in params.enterprise['users']:
                        if 'enterprise_user_id' in user and 'username' in user:
                            email = user['username']
                            username = user['data']['displayname'] if 'data' in user and 'displayname' in user['data'] else user['username']
    ...
    def execute(self, params, **kwargs):
        ...
        for raw in rows:
            ...
            for f in fields:
                row.append(raw[f])
            table.append(row)
        dump_report_data(table, fields, fmt=format, filename=kwargs.get('output'))
