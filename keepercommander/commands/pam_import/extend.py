...
from .keeper_ai_settings import (
    set_resource_jit_settings,
...
class PAMProjectExtendCommand(Command):
    ...
    def execute(self, params, **kwargs):
        ...
        if file_name:
            # Resolve symlinks so the path shown in errors is canonical
        ...
        try:
            ...
            raise CommandError("pam project extend",
                f"Invalid JSON in import file at line {e.lineno}, col {e.colno}: {e.msg}")
        except UnicodeDecodeError as e:
            raise CommandError("pam project extend", f"Import file encoding error: {e.reason}")
        except OSError as e:
    ...
    def process_folders(self, params, project: dict) -> dict:
        ...
        if logging.getLogger().getEffectiveLevel() <= logging.DEBUG:
            for path, _ in good_paths:
                tag = "existing" if path in existing_paths_set else "new"
                logging.debug(f"  [DEBUG] [{tag}] {path}")
    ...
    def map_records(self, params, project: dict) -> tuple:
        ...
        if autogenerated_titles:
            ...
            if logging.getLogger().getEffectiveLevel() <= logging.DEBUG:
                for t in autogenerated_titles:
                    logging.debug(f"  [DEBUG] autogenerated title: {t}")
        ...
        for mach in resources:
            ...
            if admin_cred:
                ...
                if len(ruids) == 1 and getattr(ruids[0], "uid", ""):
                    ...
                    logging.warning(f"{bcolors.WARNING}Multiple import records match administrative_credentials for {getattr(mach, 'title', mach)}; not resolved.{bcolors.ENDC}")
                # Fallback only when no match in import (len(ruids) == 0)
                    ...
                    if len(vault_matches) == 1:
                        ...
                        logging.warning(f"{bcolors.WARNING}Multiple vault records match administrative_credentials '{ref}' for {getattr(mach, 'title', mach)}; not resolved.{bcolors.ENDC}")
            ...
            if launch_cred and not isinstance(mach, PamRemoteBrowserObject):
                ...
                if len(ruids) == 1 and getattr(ruids[0], "uid", ""):
                    ...
                    logging.warning(f"{bcolors.WARNING}Multiple import records match launch_credentials for {getattr(mach, 'title', mach)}; not resolved.{bcolors.ENDC}")
                # Fallback only when no match in import (len(ruids) == 0)
                elif len(ruids) == 0:
                    ...
                    if len(vault_matches) == 1:
                        ...
                        logging.warning(f"{bcolors.WARNING}Multiple vault records match launch_credentials '{ref}' for {getattr(mach, 'title', mach)}; not resolved.{bcolors.ENDC}")
        ...
        if dry_run:
            for o in chain(resources, users):
                ...
                logging.info(f"  [DRY RUN] [{tag}]  folder={path}\trecord={otype}: {label}{uid_suffix}")
            for mach in resources:
                if hasattr(mach, "users") and isinstance(mach.users, list):
                    for u in mach.users:
                        ...
                        logging.info(f"  [DRY RUN] [{tag}]  folder={path}\trecord={utype}: {label} (nested on {getattr(mach, 'title', '')}){uid_suffix}")
                        # Always show in dry-run (logging.info) when user allows empty password and has none
                        if getattr(u, "_is_pam_directory_user", False) and not (getattr(u, "password", None) and str(getattr(u, "password", "") or "").strip()):
                            logging.info("PAM User (pamDirectory/AD or local user with external AD admin) has empty password (valid).")
            logging.info(f"[DRY RUN] {total_line}")
        else:
            if logging.getLogger().getEffectiveLevel() <= logging.DEBUG:
                for o in chain(resources, users):
                    tag = getattr(o, "_extend_tag", "?")
                    ...
                    label = getattr(o, "title", None) or getattr(o, "login", None) or ""
                    uid_suffix = f"\tuid={getattr(o, 'uid', '')}" if tag == "existing" else ""
                    logging.debug(f"  [DEBUG] [{tag}]  folder={path}\trecord={otype}: {label}{uid_suffix}")
                for mach in resources:
                    if hasattr(mach, "users") and isinstance(mach.users, list):
                        for u in mach.users:
                            tag = getattr(u, "_extend_tag", "?")
                            ...
                            label = getattr(u, "title", None) or getattr(u, "login", None) or ""
                            uid_suffix = f"\tuid={getattr(u, 'uid', '')}" if tag == "existing" else ""
                            # Users allowing empty password vs other users
                            if getattr(u, "_is_pam_directory_user", False) and not (getattr(u, "password", None) and str(getattr(u, "password", "") or "").strip()):
                                logging.debug(f"  [DEBUG] [{tag}]  folder={path}\trecord={utype}: {label} (nested on {getattr(mach, 'title', '')}){uid_suffix} - empty password (valid).")
                            else:
                                logging.debug(f"  [DEBUG] [{tag}]  folder={path}\trecord={utype}: {label} (nested on {getattr(mach, 'title', '')}){uid_suffix}")
            print(total_line)
