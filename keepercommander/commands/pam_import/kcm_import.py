...
SELECT
    ...
    ca.attribute_value
FROM
    guacamole_connection_attribute ca
"""
...
# Guacamole implicit defaults — applied only when KCM has no explicit port.
# Keys use the stored protocol name (postgres → postgresql after conversion).
...
class KCMDatabaseConnector:
    ...
    def extract_connections(self):
        ...
        # Fetch attributes separately to avoid N*M cartesian product
        attr_map = {}  # connection_id -> [(attr_name, attr_value)]
        try:
            self.cursor.execute(SQL_ATTRIBUTES)
            for arow in self.cursor.fetchall():
                arow = dict(arow)
                cid = arow['connection_id']
                attr_map.setdefault(cid, []).append(
                    (arow['attribute_name'], arow['attribute_value']))
        except Exception as e:
            logging.debug('Attribute query skipped (table may not exist): %s', e)

        return connection_rows, attr_map
...
class KCMParameterMapper:
    ...
    def transform(self, connection_rows, include_disabled=False, attr_map=None):
        # type: (List[Dict], bool, Dict) -> Tuple[List[Dict], List[Dict]]
        """Group rows by connection_id, apply mappings, return (resources, users).

        Parameters and attributes are fetched with separate queries to avoid
        an N*M cartesian product.  ``attr_map`` is {connection_id: [(name, value)]}.
        """
        ...
        # Apply attributes (fetched separately — no cartesian product)
        for cid, attrs in attr_map.items():
            if cid in disabled_ids or cid not in connections:
                continue
            for attr_name, attr_value in attrs:
                if attr_name:
                    self._apply_mapping(cid, attr_name,
                                        attr_value if attr_value is not None else '',
                                        connections, users)

        # Fill in default ports for connections where KCM had no explicit port.
        # Guacamole applies these defaults implicitly; Keeper needs them explicit.
        for conn in connections.values():
            conn_settings = conn['pam_settings']['connection']
            if 'port' not in conn_settings:
                default_port = PROTOCOL_DEFAULT_PORTS.get(conn_settings['protocol'])
                if default_port:
                    conn_settings['port'] = default_port
    ...
    def finalize_totp(users):
        ...
        for user in users:
            ...
            # Base32 alphabet: A-Z, 2-7 (case-insensitive, strip padding)
            stripped_secret = ''.join(
                c for c in secret.upper() if c in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567')
    ...
    def run_probe(self, params, config_uid, pam_json, extend_cmd_factory):
        ...
        for i, probe_delay in enumerate(probe_delays):
            ...
            try:
                ...
                err_msg = str(e).lower()
                if 'throttle' in err_msg or 'rate limit' in err_msg or 'too many request' in err_msg:
                    rtt = time.time() - batch_start
                    self.probe_rtts.append(rtt)
                    probe_was_throttled = True
                    logging.warning(
                        '[Probe %d/3] Throttled (%.1fs) — stopping probes',
                        i + 1, rtt)
                    break
                raise
            finally:
...
Examples:
  ...
  pam project kcm-import --docker-detect --name "Auto Import" --yes
    ...
    parser.add_argument('--docker-container', dest='docker_container',
                        ...
                        help='Docker container name (auto-discovered if not set)')
    ...
    parser.add_argument('--db-type', dest='db_type', action='store',
                        choices=['mysql', 'postgresql'], default=None,
                        help='Database type (auto-detected with --docker-detect)')
    ...
    parser.add_argument('--db-password-record', dest='db_password_record',
                        action='store',
                        help='Keeper record UID or title containing DB password')
    ...
    parser.add_argument('--config', '-c', dest='config', action='store',
                        help='Existing PAM config UID or name (extend mode)')
    ...
    parser.add_argument('--gateway', '-g', dest='gateway', action='store',
                        help='Existing gateway UID or name (interactive picker if omitted)')
    ...
    # Group filtering
    parser.add_argument('--groups', dest='include_groups', action='store',
                        default=None,
                        help='Import only these connection groups (comma-separated, '
                             'supports wildcards: "Lab*,Production")')
    parser.add_argument('--exclude-groups', dest='exclude_groups', action='store',
                        default=None,
                        help='Exclude these connection groups (comma-separated, '
                             'supports wildcards: "Incomplete*,Test*")')
    parser.add_argument('--list-groups', dest='list_groups', action='store_true',
                        default=False,
                        help='List available connection groups and exit')
    ...
    def execute(self, params, **kwargs):
        ...
        if docker_detect:
            ...
            if container_name is None:
                ...
                logging.warning('Using specified Docker container: %s',
                                container_name)
            # Auto-detect db_type if not specified
        ...
        try:
            ...
            connection_rows, attr_map = connector.extract_connections()
        except CommandError:
            raise
        except Exception as e:
        ...
        if kwargs.get('list_groups'):
            self._print_group_list(groups, resolver, resources, users)
            return

        # Interactive group picker (when no --groups flag in interactive mode)
        include_groups = kwargs.get('include_groups') or ''
        ...
        if (not include_groups and not exclude_groups
                ...
                and not kwargs.get('dry_run')
                and not kwargs.get('output')
                and not kwargs.get('estimate')
                and not getattr(params, 'batch_mode', False)):
            selected = self._interactive_group_picker(
                groups, resolver, resources, users)
            if selected is not None:
                # User selected specific groups — apply as include filter
                include_groups = selected

        # Group filtering: --groups and --exclude-groups
        if include_groups or exclude_groups:
            resources, users = self._filter_by_groups(
                resources, users, groups, resolver,
                include_groups, exclude_groups)
            num_resources = len(resources)
            num_users = len(users)
            if num_resources == 0 and num_users == 0:
                raise CommandError('kcm-import',
                    'No connections match the group filter. '
                    'Use --list-groups to see available groups.')

        # Resolve KSM dynamic tokens (${KEEPER_*}) AFTER group filtering
        # so we only process connections the user actually selected.
        ksm_warnings, ksm_resolved, ksm_unresolved = self._resolve_ksm_tokens(
            params, resources, users)
        ...
        # Move KSM-affected records to dedicated subfolders with notes
        self._flag_ksm_records(resources, users, ksm_resolved, ksm_unresolved,
                               res_root, usr_root)
        ...
        gateway_token = ''
        if not config_uid:
            config_uid, gateway_token = self._create_project_skeleton(
        ...
        # Per-record tracking: list of dicts with keys:
        #   name, type, phase ('user'|'resource'), status ('ok'|'skipped'|'error'), reason
        import_results = []
        ...
        try:
            ...
            if all_users:
                ...
                while ui < len(all_users):
                    ...
                    while True:
                        try:
                            self._run_extend_batch(
                                params, config_uid, tmp_path,
                                batch_users, 'user', import_results)
                            break
                        except Exception as e:
                            err_msg = str(e).lower()
                            if retries < 3 and ('throttle' in err_msg or 'rate limit' in err_msg or 'too many request' in err_msg):
                                retries += 1
                                wait = 60 * retries
                                logging.warning(
                                ...
                                throttler.record_batch(
                                    wait, len(batch_users),
                                    is_resource=False)
                            else:
                                # Mark batch items as errors
                                for item in batch_users:
                                    import_results.append({
                                        'name': item.get('title', item.get('login', '')),
                ...
                while ri < len(all_resources):
                    ...
                    while True:
                        try:
                            self._run_extend_batch(
                                params, config_uid, tmp_path,
                                batch_resources, 'resource', import_results)
                            break
                        except Exception as e:
                            err_msg = str(e).lower()
                            if retries < 3 and ('throttle' in err_msg or 'rate limit' in err_msg or 'too many request' in err_msg):
                                ...
                                time.sleep(wait)
                                throttler.record_batch(
                                    wait, len(batch_resources),
                                    is_resource=True)
                            else:
                                for item in batch_resources:
                                    import_results.append({
                                        'name': item.get('title', ''),
                                        'type': item.get('type', 'unknown'),
                                        'phase': 'resource',
            ...
            # Gather project assets for report
            assets = self._get_project_assets(params, config_uid)
            if gateway_token:
                assets['gateway_token'] = gateway_token
            assets['config_uid'] = config_uid

            # Build and print structured report
            report_text = self._build_import_report(
            ...
            try:
                ...
                self._create_summary_record(
                    params, res_sf_name, report_text,
                    project_name=project_name, assets=assets,
                    kwargs=kwargs,
                    import_results=import_results,
                    warnings=warnings)
            except Exception as e:
                logging.warning('Could not create summary record: %s', e)
                logging.debug('Summary record traceback:', exc_info=True)
        finally:
            if tmp_path and os.path.exists(tmp_path):
                os.unlink(tmp_path)

    @staticmethod
    def _run_extend_batch(params, config_uid, tmp_path, batch_items,
                          phase, import_results):
        """Run a single extend batch and capture per-record results.

        Parses stdout from the extend command to determine which records
        were created, skipped (existing), or failed (missing fields).

        Args:
            batch_items: list of dicts from the import JSON (resources or users)
            phase: 'user' or 'resource'
            import_results: list to append result dicts to
        """
        from .extend import PAMProjectExtendCommand
        ...
        #   "PAM User is missing required field `password`"
        #   "N existing records (skipped), M new records to be created"
        missing_fields = []
        existing_count = 0
        new_count = 0
        for line in batch_output.splitlines():
            stripped = line.strip()
            if 'missing required field' in stripped.lower():
                missing_fields.append(stripped)
            if 'existing records' in stripped and 'skipped' in stripped:
                m = re.search(r'(\d+)\s+existing records?\s*\(skipped\)', stripped)
                if m:
                    existing_count += int(m.group(1))
            if 'new records to be created' in stripped:
                m = re.search(r'(\d+)\s+new records? to be created', stripped)
                if m:
                    new_count += int(m.group(1))

        # Build per-item results (includes nested users inside resources)
        all_items = []
        for item in batch_items:
            all_items.append(item)
            # Resources can have nested users that also become records
            for nested_user in item.get('users', []):
                all_items.append(nested_user)

        for item in all_items:
            title = item.get('title', item.get('login', ''))
            rtype = item.get('type', 'unknown')
            # Check if this item had a missing-field warning
            item_failed = False
            fail_reason = ''
            for mf in missing_fields:
                # Missing field warnings don't name the record, so we
                # attribute to items missing the mentioned field
                if 'password' in mf.lower():
            ...
            if item_failed:
                import_results.append({
                    'name': title, 'type': rtype, 'phase': phase,
                    'status': 'skipped', 'reason': fail_reason,
                })
            else:
                import_results.append({
                    'name': title, 'type': rtype, 'phase': phase,
    ...
    def _compute_batch_params(num_resources, num_users,
        ...
        # Allow CLI overrides
        if override_size is not None and override_size > 0:
    ...
    @staticmethod
    def _print_group_list(groups, resolver, resources, users):
        """Print available connection groups with resource counts."""
        # Build group_id → name mapping
        group_names = {}
        for g in groups:
            gid = g['connection_group_id']
            group_names[gid] = g.get('connection_group_name', f'group_{gid}')
        ...
        for r in resources:
            gid = r.get('_group_id')
            group_res_counts[gid] = group_res_counts.get(gid, 0) + 1
        for u in users:
            gid = u.get('_group_id')
            group_usr_counts[gid] = group_usr_counts.get(gid, 0) + 1

        # Also count items in ROOT (no group)
        root_res = group_res_counts.get(None, 0)
        root_usr = group_usr_counts.get(None, 0)

        print('\nAvailable Connection Groups:')
        print('=' * 70)
        print(f'  {"Group Name":<45s} {"Resources":>10s} {"Users":>8s}')
        print('  ' + '-' * 65)

        if root_res or root_usr:
            print(f'  {"(ROOT - no group)":<45s} {root_res:>10d} {root_usr:>8d}')

        for g in sorted(groups, key=lambda x: x.get('connection_group_name', '')):
            gid = g['connection_group_id']
            name = g.get('connection_group_name', f'group_{gid}')
            path = resolver.resolve_path(gid)
            res_n = group_res_counts.get(gid, 0)
            usr_n = group_usr_counts.get(gid, 0)
            # Only show groups that have connections (or are parents)
            indent = '  '
            if '/' in path:
                depth = path.count('/')
                indent = '  ' + '  ' * depth
            ...
            if res_n > 0 or usr_n > 0:
                print(f'  {display:<45s} {res_n:>10d} {usr_n:>8d}')
            else:
                print(f'  {display:<45s} {"—":>10s} {"—":>8s}')

        total_res = len(resources)
        total_usr = len(users)
        print('  ' + '-' * 65)
        print(f'  {"TOTAL":<45s} {total_res:>10d} {total_usr:>8d}')
        print()
        print('  Use --groups "Name1,Name2" to import specific groups')
        print('  Use --exclude-groups "Name1,Name2" to exclude groups')
        print('  Wildcards supported: --exclude-groups "Incomplete*,Test*"')
        print()

    @staticmethod
    def _interactive_group_picker(groups, resolver, resources, users):
        """Show connection groups and let user select which to import.

        Returns a comma-separated pattern string for _filter_by_groups,
        or None if user wants to import all groups.
        """
        # Build group stats
        group_res = {}
        group_usr = {}
        for r in resources:
            gid = r.get('_group_id')
            group_res[gid] = group_res.get(gid, 0) + 1
        for u in users:
            gid = u.get('_group_id')
            group_usr[gid] = group_usr.get(gid, 0) + 1

        # Only show top-level groups (no parent or parent is ROOT)
        top_groups = []
        for g in groups:
            path = resolver.resolve_path(g['connection_group_id'])
            # Top-level: path is "ROOT/name" or just the group name (ksm mode)
            depth = path.count('/')
            if depth <= 1:
                gid = g['connection_group_id']
                name = g.get('connection_group_name', f'group_{gid}')
                # Count all items in this group and its children
                res_n = 0
                usr_n = 0
                for g2 in groups:
                    g2_path = resolver.resolve_path(g2['connection_group_id'])
                    if g2_path == path or g2_path.startswith(path + '/'):
        ...
        root_res = group_res.get(None, 0)
        root_usr = group_usr.get(None, 0)

        print('\nConnection Groups Found:')
        print('─' * 60)
        idx = 1
        numbered = []
        if root_res or root_usr:
            print(f'  [{idx}] (ROOT — ungrouped)  '
                  f'({root_res} resources, {root_usr} users)')
            numbered.append('ROOT')
            idx += 1
        for name, res_n, usr_n in sorted(top_groups):
            print(f'  [{idx}] {name}  '
                  f'({res_n} resources, {usr_n} users)')
        ...
        try:
            choice = input('  Select groups (comma-separated numbers, or A for all) [A]: ').strip()
        except EOFError:
            return None

        if not choice or choice.upper() == 'A':
            return None  # import all
        ...
        for part in choice.split(','):
            ...
            try:
                i = int(part) - 1
                if 0 <= i < len(numbered):
                    selected_names.append(numbered[i])
            except ValueError:
                continue

        if not selected_names:
            return None  # bad input, import all

        # Build pattern string — exact names (no wildcards needed)
        # Use wildcards to match children too
        patterns = [f'{name}*' if name != 'ROOT' else name
                    for name in selected_names]
        logging.warning('Selected groups: %s', ', '.join(selected_names))
        return ','.join(patterns)

    @staticmethod
    def _filter_by_groups(resources, users, groups, resolver,
                          include_pattern, exclude_pattern):
        """Filter resources and users by connection group name patterns.

        Patterns are comma-separated, support fnmatch wildcards (* and ?).
        Returns (filtered_resources, filtered_users).
        """
        from fnmatch import fnmatch

        # Build group_id → group_name and group_id → full_path
        group_names = {}
        group_paths = {}
        for g in groups:
            gid = g['connection_group_id']
            group_names[gid] = g.get('connection_group_name', '')
            group_paths[gid] = resolver.resolve_path(gid)

        include_pats = [p.strip() for p in include_pattern.split(',')
                        if p.strip()] if include_pattern else []
        exclude_pats = [p.strip() for p in exclude_pattern.split(',')
                        if p.strip()] if exclude_pattern else []

        def _matches(gid, patterns):
            """Check if a group matches any of the patterns."""
            name = group_names.get(gid, '')
            path = group_paths.get(gid, '')
            for pat in patterns:
                pat_lower = pat.lower()
                # Match against group name, full path, or path segments
                if fnmatch(name.lower(), pat_lower):
                    return True
                if fnmatch(path.lower(), pat_lower):
                    return True
                # Also match any path segment
                for segment in path.split('/'):
                    if fnmatch(segment.lower(), pat_lower):
                        return True
            return False

        def _keep(item):
            gid = item.get('_group_id')
            if gid is None:
                # ROOT items: keep unless --groups is set, or if ROOT was
                # explicitly selected in the interactive picker
                if not include_pats:
                    return True
                return any(p == 'ROOT' for p in include_pats)
            if include_pats and not _matches(gid, include_pats):
                return False
            if exclude_pats and _matches(gid, exclude_pats):
                return False
            return True

        filtered_res = [r for r in resources if _keep(r)]
        filtered_usr = [u for u in users if _keep(u)]

        kept = len(filtered_res) + len(filtered_usr)
    ...
    def _validate_import_data(resources, users, skip_users):
        ...
        # Check for users missing passwords (will be skipped by extend engine)
        if not skip_users:
            no_pw_external = [u for u in users if not u.get('password')]
            no_pw_nested = []
            for r in resources:
                for nu in r.get('users', []):
                    if not nu.get('password'):
                        no_pw_nested.append(nu)
            total_no_pw = len(no_pw_external) + len(no_pw_nested)
            if total_no_pw > 0:
                warnings.append(
                    f'{total_no_pw} user(s) missing password '
                    f'({len(no_pw_external)} external, {len(no_pw_nested)} nested) '
                    f'— these will be created without credentials')

        # Check for resources missing host (skip pamRemoteBrowser — uses URL not host)
        no_host = [r for r in resources
                   if not r.get('host') and r.get('type') != 'pamRemoteBrowser']
        if no_host:
            for r in no_host:
                warnings.append(
    ...
    @staticmethod
    def _resolve_ksm_tokens(params, resources, users):
        """Resolve ${KEEPER_*} dynamic tokens from KCM vault integration.

        KCM servers with KSM integration store placeholder tokens
        (e.g., ${KEEPER_USER_PASSWORD}) instead of real credentials.  This
        method detects those tokens and handles them correctly:

        - If the credential record already exists in the user's vault,
          LINK to it (via uid → uid_imported) instead of duplicating it.
        - If not found, clear the token (never import literal ${KEEPER_*}
          strings) and warn the user.

        Matching logic (mirrors KCM's runtime resolution):
        - ${KEEPER_SERVER_*} tokens: matched by hostname → vault record host
        - ${KEEPER_USER_*} tokens:   matched by username → vault record login
        - ${KEEPER_DOMAIN_*} tokens: matched by domain   → vault record domain

        Returns a list of warning strings for tokens that couldn't be resolved.
        """
        _TOKEN_RE = re.compile(r'^\$\{KEEPER_(\w+)\}$')

        # Check if any tokens exist before building the expensive index
        has_tokens = False
        for resource in resources:
            conn = resource.get('pam_settings', {}).get('connection', {})
            for val in conn.values():
                if isinstance(val, str) and val.startswith('${KEEPER_'):
                    has_tokens = True
                    break
            if has_tokens:
                break
        if not has_tokens:
            for user in users:
                for key in ('password', 'login', 'private_pem_key'):
                    val = user.get(key, '')
                    if isinstance(val, str) and val.startswith('${KEEPER_'):
                        has_tokens = True
                        break
        ...
        logging.warning(
            f'{bcolors.WARNING}KSM integration detected:{bcolors.ENDC} '
            f'some connections use ${{KEEPER_*}} dynamic tokens. '
            f'Attempting to resolve from vault...')

        # Build vault lookup indexes from all accessible records
        vault_by_host = {}   # host/IP → {uid, title, login, password, ...}
        vault_by_login = {}  # login → {uid, title, password, ...}
        for uid in params.record_cache:
            try:
                rec = vault.KeeperRecord.load(params, uid)
                if not rec:
                    continue
                rec_data = {'uid': uid, 'title': getattr(rec, 'title', '')}

                if hasattr(rec, 'get_typed_field'):
                    for ftype in ('login', 'password', 'host', 'keyPair',
                                  'oneTimeCode'):
                        field = rec.get_typed_field(ftype)
                        if field and field.value:
                            ...
                            if isinstance(val, dict):
                                ...
                                rec_data[ftype] = val
                elif hasattr(rec, 'login'):
                    rec_data['login'] = getattr(rec, 'login', '') or ''
                    rec_data['password'] = getattr(rec, 'password', '') or ''

                host = rec_data.get('host', '')
                if host:
                    host_key = host.split(':')[0].strip().lower()
                    if host_key and host_key not in vault_by_host:
                        vault_by_host[host_key] = rec_data
                    elif host_key:
                        logging.debug(
                            'Multiple vault records for host %s: %s and %s',
                            host_key, vault_by_host[host_key].get('title'),
                            rec_data.get('title'))

                login = rec_data.get('login', '')
                if login:
                    login_key = login.strip().lower()
                    if login_key and login_key not in vault_by_login:
                        vault_by_login[login_key] = rec_data
                    elif login_key:
                        logging.debug(
                            'Multiple vault records for login %s: %s and %s',
                            login_key, vault_by_login[login_key].get('title'),
                            rec_data.get('title'))
            except Exception:
                continue

        warnings = []
        resolved_details = []   # (resource_title, host, login, vault_title)
        unresolved_details = [] # (resource_title, protocol, host, login, token, field)

        # Track which users have been linked to existing vault records
        linked_users = set()  # set of user titles

        for resource in resources:
            host = resource.get('host', '')
            conn = resource['pam_settings']['connection']
            res_name = resource.get('title', '').replace('KCM Resource - ', '')

            # Find the paired user for this resource
            paired_user = None
            for u in users:
                if u.get('title') == f'KCM User - {res_name}':
                    paired_user = u
                    break

            # Scan connection fields for tokens
            has_resource_token = False
            for key in list(conn.keys()):
                ...
                if not m:
                    continue
                has_resource_token = True

                token_name = m.group(1)
                vault_rec = None

                if token_name.startswith('SERVER_'):
                    host_key = host.strip().lower()
                    vault_rec = vault_by_host.get(host_key)
                elif token_name.startswith('USER_'):
                    if paired_user:
                        login_key = paired_user.get('login', '').lower()
                        vault_rec = vault_by_login.get(login_key)
                elif token_name.startswith('DOMAIN_'):
                    domain = resource.get('domain_name', '').lower()
                    vault_rec = vault_by_host.get(domain)

                if vault_rec:
                    conn[key] = ''
                else:
                    protocol = conn.get('protocol', '')
                    login = paired_user.get('login', '') if paired_user else ''
                    unresolved_details.append((
                        res_name, protocol, host, login, val, key))
                    conn[key] = ''

            # If we found a vault record for the user tokens, LINK instead
            # of creating a duplicate.  Set uid_imported on the paired user
            # so extend.py uses the existing vault record.
            if has_resource_token and paired_user:
                login_key = paired_user.get('login', '').lower()
                vault_rec = vault_by_login.get(login_key)
                if not vault_rec:
                    # Try by host (SERVER tokens)
                    host_key = host.strip().lower()
                    vault_rec = vault_by_host.get(host_key)

                if vault_rec and vault_rec.get('uid'):
                    linked_users.add(paired_user.get('title', ''))
                    resolved_details.append((
                        res_name, host,
                        paired_user.get('login', ''),
                        vault_rec.get('title', ''),
                        vault_rec['uid']))
                    logging.warning(
                        f'{bcolors.OKBLUE}KSM match:{bcolors.ENDC} %s '
                        f'(login=%s, host=%s) → vault record "%s" (%s)',
                        res_name,
                        paired_user.get('login', ''),
                        host or '(none)',
                        vault_rec.get('title', ''),
            ...
            # Check host field itself for tokens
            if host and _TOKEN_RE.match(host):
                protocol = conn.get('protocol', '')
                login = paired_user.get('login', '') if paired_user else ''
                unresolved_details.append((
                    res_name, protocol, '(token)', login, host, 'host'))
                resource['host'] = ''

        # Process user-level tokens (password, login, private_pem_key)
        for user in users:
            user_title = user.get('title', '')
            if user_title in linked_users:
                # Already linked to an existing vault record — clear any
                # remaining tokens but don't try to resolve (the existing
                # record has the real values)
                for key in ('password', 'login', 'private_pem_key'):
                    val = user.get(key, '')
                    if isinstance(val, str) and _TOKEN_RE.match(val):
                        user[key] = ''
                continue
            ...
            for key in ('password', 'login', 'private_pem_key'):
                ...
                if token_name.startswith('USER_'):
                    login_key = user.get('login', '').lower()
                    vault_rec = vault_by_login.get(login_key)
                elif token_name.startswith('SERVER_'):
                    usr_name = user_title.replace('KCM User - ', '')
                    for r in resources:
                        if r.get('title') == f'KCM Resource - {usr_name}':
                            host_key = r.get('host', '').strip().lower()
                            vault_rec = vault_by_host.get(host_key)
                            break
                ...
                if vault_rec:
                    ...
                    # Find paired resource for context
                    usr_name = user_title.replace('KCM User - ', '')
                    res_host = ''
                    res_proto = ''
                    for r in resources:
                        if r.get('title') == f'KCM Resource - {usr_name}':
                            res_host = r.get('host', '')
                            res_proto = r.get('pam_settings', {}).get(
                                'connection', {}).get('protocol', '')
                            break
                    unresolved_details.append((
                        usr_name, res_proto, res_host,
                        user.get('login', ''), val, key))
                    user[key] = ''

        # Build warnings with structured detail
        if resolved_details:
            logging.warning(
                f'{bcolors.OKGREEN}KSM tokens:{bcolors.ENDC} %d connection(s) '
                f'matched existing vault records — tokens cleared',
                len(resolved_details))
            # Build a resolved summary for the report
            resolved_lines = []
            for name, host, login, vtitle, vuid in resolved_details:
                resolved_lines.append(
                    f'{name} (login={login}, host={host or "(none)"}) '
                    f'→ "{vtitle}"')
            warnings.append(
                f'{len(resolved_details)} KSM connection(s) matched existing '
                f'vault records (tokens cleared, credentials on existing '
                f'record):\n' + '\n'.join(
        ...
        if unresolved_details:
            ...
            logging.warning(
                f'{bcolors.WARNING}KSM tokens:{bcolors.ENDC} %d connection(s) '
                f'could not be resolved — credentials imported empty',
                len(unique))
            # Build a structured warning with actionable info
            unresolved_lines = []
            for name, proto, host, login, token, field in unique:
                parts = [name]
                if proto:
                ...
                unresolved_lines.append(' | '.join(parts))

            warnings.append(
                f'{len(unique)} KSM connection(s) need credentials added '
                f'manually. These used ${{KEEPER_*}} tokens in KCM but no '
    ...
    @staticmethod
    def _flag_ksm_records(resources, users, resolved_details, unresolved_details,
                          res_root, usr_root):
        """Add notes and move KSM-affected records to dedicated subfolders.

        Resolved records stay in place but get a note explaining the match.
        Unresolved records move to a mirrored 'Needs KSM Credentials/' subfolder
        so they're visually separated and easy to find.
        """
        if not resolved_details and not unresolved_details:
            return

        # Build lookup: connection name → resolved detail
        resolved_map = {}
        for name, host, login, vtitle, vuid in resolved_details:
            resolved_map[name] = (host, login, vtitle, vuid)

        # Build lookup: connection name → unresolved detail
        unresolved_map = {}
        for name, proto, host, login, token, field in unresolved_details:
            if name not in unresolved_map:
                unresolved_map[name] = (proto, host, login, token)

        # Add notes to resolved user records
        for user in users:
            conn_name = user.get('title', '').replace('KCM User - ', '')
            detail = resolved_map.get(conn_name)
            if detail:
                host, login, vtitle, vuid = detail
                note = (
                    f'[KSM Integration] This connection used dynamic tokens in KCM.\n'
                    f'Matched vault record: "{vtitle}" (UID: {vuid})\n'
                    f'Original token: ${{KEEPER_USER_PASSWORD}}\n'
                    f'The credential was not copied — add it manually or link '
                    f'to the existing record.')
                existing = user.get('notes', '')
                user['notes'] = f'{existing}\n\n{note}'.strip() if existing else note

        # Move unresolved user records to mirrored subfolder + add notes
        ksm_usr_root = f'{usr_root}/Needs KSM Credentials'
        ksm_res_root = f'{res_root}/Needs KSM Credentials'

        for user in users:
            conn_name = user.get('title', '').replace('KCM User - ', '')
            detail = unresolved_map.get(conn_name)
            if not detail:
                continue
            proto, host, login, token = detail

            # Mirror the folder path: replace usr_root with ksm subfolder
            orig_path = user.get('folder_path', '')
            if orig_path.startswith(usr_root):
                suffix = orig_path[len(usr_root):]
                user['folder_path'] = f'{ksm_usr_root}{suffix}'
                user['_original_folder'] = orig_path

            note = (
                f'[KSM Integration — Needs Credentials]\n'
        ...
        # Also move the paired resource records for unresolved connections
        for resource in resources:
            conn_name = resource.get('title', '').replace('KCM Resource - ', '')
            if conn_name not in unresolved_map:
                continue
            orig_path = resource.get('folder_path', '')
            if orig_path.startswith(res_root):
                suffix = orig_path[len(res_root):]
                resource['folder_path'] = f'{ksm_res_root}{suffix}'
                resource['_original_folder'] = orig_path

            proto, host, login, token = unresolved_map[conn_name]
            note = (
                f'[KSM Integration — Needs Credentials]\n'
    ...
    @staticmethod
    def _get_project_assets(params, config_uid):
        """Gather project asset UIDs/names for the import report."""
        assets = {}  # type: Dict[str, str]
        try:
            # Get shared folder names
            actual_res, actual_usr = \
                PAMProjectKCMImportCommand._discover_shared_folder_names(
                    params, config_uid)
            assets['res_sf_name'] = actual_res or ''
            assets['usr_sf_name'] = actual_usr or ''

            # Find SF UIDs
            for sf_uid, sf in params.shared_folder_cache.items():
                name = sf.get('name_unencrypted', '')
                if name == actual_res:
                    assets['res_sf_uid'] = sf_uid
                elif name == actual_usr:
                    assets['usr_sf_uid'] = sf_uid

            # Get gateway UID from PAM config's pamResources field
            config_rec = vault.KeeperRecord.load(params, config_uid)
            if config_rec and hasattr(config_rec, 'fields'):
                pam_res = next(
                    (f for f in config_rec.fields
                     if f.type == 'pamResources'), None)
                if pam_res and pam_res.value:
                    val = pam_res.value[0] if isinstance(
                        pam_res.value, list) else pam_res.value
                    if isinstance(val, dict):
                        gw_uid = val.get('controllerUid', '')
                        if gw_uid:
                            assets['gateway_uid'] = gw_uid
                            from ..pam import gateway_helper
                            gateways = gateway_helper.get_all_gateways(params)
                            for g in gateways:
                                g_uid = utils.base64_url_encode(
                                    g.controllerUid)
                                if g_uid == gw_uid:
                                    assets['gateway_name'] = \
                                        g.controllerName
            ...
            logging.debug('Could not gather project assets: %s', e)

        return assets

    @staticmethod
    def _build_import_report(project_name, config_uid, is_new_project,
                             assets, num_resources, num_users, created,
                             expected, total_time, throttler_summary,
                             warnings, kwargs, import_results=None):
        """Build a structured import report for console and vault storage."""
        ts = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        skipped = expected - created
        mins = total_time / 60

        lines = [
            '',
            '=' * 60,
            f' KCM Import Complete — {project_name}',
            '=' * 60,
            '',
            f' Timestamp:  {ts}',
            f' Config UID: {config_uid}',
            f' Mode:       {"New project" if is_new_project else "Extend existing"}',
            '',
            ' PROJECT ASSETS',
            ' ' + '-' * 40,
        ]

        gw_name = assets.get('gateway_name', 'Unknown')
        gw_uid = assets.get('gateway_uid', '')
        gw_label = 'NEW' if is_new_project else 'EXISTING'
        lines.append(f'   Gateway:    {gw_name} ({gw_uid}) [{gw_label}]')

        app_uid = assets.get('app_uid', '')
        if app_uid:
            lines.append(f'   KSM App:    {app_uid}')

        res_sf = assets.get('res_sf_name', '')
        res_uid = assets.get('res_sf_uid', '')
        if res_sf:
            lines.append(f'   Resources:  {res_sf} ({res_uid})')
        usr_sf = assets.get('usr_sf_name', '')
        usr_uid = assets.get('usr_sf_uid', '')
        if usr_sf:
            lines.append(f'   Users:      {usr_sf} ({usr_uid})')
        ...
        if skipped > 0:
            lines.append(f'   Skipped:    {skipped} (missing required fields)')
        lines.append(f'   Duration:   {total_time:.0f}s ({mins:.1f}m)')

        # Per-record detail
        if import_results:
            ok_items = [r for r in import_results if r['status'] == 'ok']
            skip_items = [r for r in import_results if r['status'] == 'skipped']
            err_items = [r for r in import_results if r['status'] == 'error']

            if skip_items or err_items:
                lines.extend([
                    '',
                ...
                for r in skip_items:
                    lines.append(
                        f'   SKIP  {r["type"]:20s} {r["name"][:40]}'
                        f'  — {r["reason"][:60]}')
                for r in err_items:
                    lines.append(
                        f'   ERR   {r["type"]:20s} {r["name"][:40]}'
                        f'  — {r["reason"][:60]}')
            ...
            if type_counts:
                lines.extend([
                    ...
                    ' RECORD BREAKDOWN',
                    ' ' + '-' * 40,
                    f'   {"Type":<22s} {"OK":>5s} {"Skip":>5s} {"Err":>5s}',
                ])
                all_types = sorted(set(r['type'] for r in import_results))
                for rtype in all_types:
                    ok_n = type_counts.get((rtype, 'ok'), 0)
                    sk_n = type_counts.get((rtype, 'skipped'), 0)
                    er_n = type_counts.get((rtype, 'error'), 0)
                    lines.append(
                        f'   {rtype:<22s} {ok_n:>5d} {sk_n:>5d} {er_n:>5d}')
                totals = (len(ok_items), len(skip_items), len(err_items))
                lines.append(
                    f'   {"TOTAL":<22s} {totals[0]:>5d} '
                    f'{totals[1]:>5d} {totals[2]:>5d}')

        # Throttle stats
        if throttler_summary:
            tc = throttler_summary.get('throttle_count', 0)
            tb = throttler_summary.get('total_batches', 0)
            rtt = throttler_summary.get('base_rtt')
            lines.extend([
                '',
                ' THROTTLE STATISTICS',
                ' ' + '-' * 40,
                f'   Throttle events: {tc}',
                f'   Total batches:   {tb}',
                f'   Final params:    res='
                f'{throttler_summary.get("final_res_batch", "?")}@'
                f'{throttler_summary.get("final_res_delay", 0):.0f}s, '
                f'usr={throttler_summary.get("final_usr_batch", "?")}@'
                f'{throttler_summary.get("final_usr_delay", 0):.0f}s',
            ])
            if rtt:
                lines.append(f'   Probe RTT:       {rtt:.1f}s')
        ...
        # Gateway deployment (new only)
        if is_new_project:
            gw_token = assets.get('gateway_token', '')
            token_display = gw_token if gw_token else '<access_token>'
            lines.extend([
                '',
                ' GATEWAY DEPLOYMENT',
                ' ' + '-' * 40,
            ...
            if gw_token:
                ...
                lines.append('   (Token not captured — check console output above)')
                lines.append('')
            lines.extend([
                '   docker run -d --name keeper-gateway \\',
                f'     -e GATEWAY_CONFIG="{token_display}" \\',
                '     -e ACCEPT_EULA=Y \\',
                '     --shm-size=2g \\',
                '     --restart unless-stopped \\',
                '     keeper/gateway:latest',
            ])
        ...
        lines.extend([
            ...
            ' ' + '-' * 40,
            '',
            '   1. DEPLOY THE GATEWAY (if new project)',
            '      Run the docker command on a host that can reach your managed machines.',
            '      Best practice: deploy on the same host as KCM, or any server with',
            '      network access to the target machines. If KCM runs in Docker, join',
            '      the same Docker network (--network <kcm_network_name>).',
            '      Copy the "Deploy Gateway" field from this record and run it.',
            '',
            '   2. VERIFY GATEWAY IS ONLINE',
            '      Run: pam gateway list',
            '      The gateway should appear as "connected" within 60 seconds.',
            '',
            '   3. ADD MISSING CREDENTIALS',
        ])
        # Check for KSM token warnings
        ksm_affected = [w for w in warnings if 'KSM dynamic token' in w]
        missing_pw = [w for w in warnings if 'missing password' in w]
        if ksm_affected or missing_pw:
            lines.append('      Some records were imported without credentials:')
            if ksm_affected:
                lines.append('      - KSM token connections: see WARNINGS below for affected records.')
                lines.append('        These had ${KEEPER_*} tokens in KCM. Add passwords manually')
                lines.append('        or share the KSM app folder with your account and re-import.')
            if missing_pw:
                lines.append('      - Records with no password in KCM source: add credentials in vault.')
        else:
            lines.append('      All credentials imported successfully.')
        lines.extend([
            '',
            '   4. TEST A CONNECTION',
            '      Open a PAM connection in the vault to verify it works.',
            '',
            '   5. CLEAN UP (when done testing)',
            f'      Dry run:  pam project kcm-cleanup --name "{project_name}" --dry-run',
            f'      Delete:   pam project kcm-cleanup --name "{project_name}" --yes',
            '',
            '   6. ADD NEW CONNECTIONS LATER',
            f'      Extend:   pam project kcm-import --config "{config_uid}" --docker-detect --yes',
            '      This adds only NEW connections (existing records are skipped).',
        ])

        # Command used (redacted)
        redacted = PAMProjectKCMImportCommand._build_redacted_command(kwargs)
        lines.extend([
            '',
            ' COMMAND USED (redacted)',
            ' ' + '-' * 40,
    ...
    @staticmethod
    def _build_redacted_command(kwargs):
        """Reconstruct the CLI command from kwargs, redacting secrets."""
        parts = ['pam project kcm-import']
        flag_map = {
            'db_host': '--db-host',
            'db_port': '--db-port',
            'db_name': '--db-name',
        ...
        bool_flags = {
            ...
            'include_credentials': '--include-credentials',
            'auto_confirm': '--yes',
            'estimate': '--estimate',
            'list_groups': '--list-groups',
            'db_ssl': '--db-ssl',
            'allow_cleartext': '--allow-cleartext',
        }
        for kwarg, flag in flag_map.items():
        ...
        for kwarg, flag in bool_flags.items():
            if kwargs.get(kwarg):
                parts.append(flag)
        if kwargs.get('db_password_record'):
            parts.append('--db-password-record "[REDACTED]"')
        return ' '.join(parts)

    @staticmethod
    def _create_summary_record(params, res_folder_name, report_text,
                               project_name='', assets=None, kwargs=None,
                               import_results=None, warnings=None):
        """Create an import report record with copyable fields and
        the full report as a file attachment.

        Placed at the project's top-level folder (alongside Resources/Users).
        Key values (gateway token, config UID, docker command) are stored as
        separate custom fields so users can copy them directly from the vault UI.
        """
        ...
        # Sync vault to pick up folders created during import
        ...
        ts = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        title = f'KCM Import Report - {ts}'

        # Custom fields for easy copy from vault UI
        gw_token = assets.get('gateway_token', '')
        config_uid = assets.get('config_uid', '')
        gw_name = assets.get('gateway_name', '')
        gw_uid = assets.get('gateway_uid', '')
        app_uid = assets.get('app_uid', '')
        res_sf = assets.get('res_sf_name', '')
        res_sf_uid = assets.get('res_sf_uid', '')
        usr_sf = assets.get('usr_sf_name', '')
        usr_sf_uid = assets.get('usr_sf_uid', '')

        docker_cmd = (
            'docker run -d --name keeper-gateway '
            '-e GATEWAY_CONFIG="<paste token>" '
            '-e ACCEPT_EULA=Y --shm-size=2g '
            '--restart unless-stopped keeper/gateway:latest'
        )
        ...
        if gw_token:
            fields.append(
                f'c.password.Gateway Token — paste into GATEWAY_CONFIG={gw_token}')
            deploy_cmd = (
                f'docker run -d --name keeper-gateway '
                f'-e GATEWAY_CONFIG="{gw_token}" '
                f'-e ACCEPT_EULA=Y --shm-size=2g '
                f'--restart unless-stopped keeper/gateway:latest'
            )
            fields.append(
                f'c.password.Deploy Gateway — copy & run in terminal={deploy_cmd}')
        else:
            fields.append(
                f'c.text.Deploy Gateway — copy & run in terminal={docker_cmd}')
        ...
        if config_uid:
            fields.append(
                f'c.text.PAM Config UID — use with --config flag={config_uid}')

        # --- Action commands ---
        fields.append(
            'c.text.Verify Gateway — run after deploy=pam gateway list')

        if config_uid:
            extend_parts = [f'pam project kcm-import --config "{config_uid}"']
            if kwargs.get('docker_detect'):
                extend_parts.append('--docker-detect')
            elif kwargs.get('db_host'):
                extend_parts.append(f'--db-host "{kwargs["db_host"]}"')
                if kwargs.get('db_type'):
                    extend_parts.append(f'--db-type {kwargs["db_type"]}')
            if kwargs.get('include_groups'):
                extend_parts.append(f'--groups "{kwargs["include_groups"]}"')
            extend_parts.append('--yes')
            fields.append(
                f'c.text.Add New Connections — re-scan KCM, skip existing='
                f'{" ".join(extend_parts)}')

        if project_name:
            fields.append(
                f'c.text.Preview Cleanup — shows what will be deleted='
                f'pam project kcm-cleanup --name "{project_name}" --dry-run')
            fields.append(
                f'c.text.Delete Project — destructive, removes all imported records='
                f'pam project kcm-cleanup --name "{project_name}" --yes')

        redacted = PAMProjectKCMImportCommand._build_redacted_command(kwargs)
        fields.append(
            f'c.text.Original Import Command — what was used for this import='
            f'{redacted}')

        reimport_parts = [redacted.replace('--dry-run', '').strip()]
        if '--yes' not in reimport_parts[0]:
            reimport_parts.append('--yes')
        fields.append(
            f'c.text.Re-import Everything — fresh run, same settings='
            f'{" ".join(reimport_parts)}')

        # --- Source & stats ---
        db_source_parts = []
        if kwargs.get('docker_detect'):
            db_source_parts.append('Docker auto-detect')
            if kwargs.get('docker_container'):
                db_source_parts.append(f'container={kwargs["docker_container"]}')
        if kwargs.get('db_host'):
            db_source_parts.append(f'host={kwargs["db_host"]}')
        if kwargs.get('db_type'):
            db_source_parts.append(f'type={kwargs["db_type"]}')
        if kwargs.get('db_name'):
            db_source_parts.append(f'db={kwargs["db_name"]}')
        if kwargs.get('db_port'):
            db_source_parts.append(f'port={kwargs["db_port"]}')
        if db_source_parts:
            fields.append(
                f'c.text.KCM Database Source={", ".join(db_source_parts)}')

        if import_results:
            ok = sum(1 for r in import_results if r['status'] == 'ok')
            skipped = sum(1 for r in import_results if r['status'] == 'skipped')
            errors = sum(1 for r in import_results if r['status'] == 'error')
            fields.append(
                f'c.text.Import Results=Created: {ok}, Skipped: {skipped}, Errors: {errors}')

            problem_items = [r for r in import_results if r['status'] != 'ok']
            if problem_items:
                detail_lines = [f"{r['name']} ({r['status']}: {r.get('reason', '')})"
                                for r in problem_items[:15]]
                if len(problem_items) > 15:
                    detail_lines.append(f'(+{len(problem_items)-15} more)')
                fields.append(
                    f'c.text.Failed Records={"; ".join(detail_lines)}')

        # --- KSM affected connections ---
        ksm_unresolved = [w for w in (warnings or [])
                          if 'need credentials added' in w]
        ksm_resolved = [w for w in (warnings or [])
                        if 'matched existing vault records' in w]
        if ksm_unresolved:
            fields.append(
                f'c.text.Needs Credentials — had KSM tokens, no vault match='
                f'{ksm_unresolved[0]}')
        if ksm_resolved:
            fields.append(
                f'c.text.KSM Resolved — matched existing vault records='
                f'{ksm_resolved[0]}')

        # --- Step-by-step action plan (each step is a separate field) ---
        step = 1
        if gw_token:
            fields.append(
                f'c.text.Step {step}: Deploy Gateway — run the "Deploy Gateway" command above='
                f'Copy the "Deploy Gateway" field and run it in your terminal. '
                f'The gateway should appear as "connected" within 60 seconds.')
            step += 1

        fields.append(
            f'c.text.Step {step}: Verify Gateway is online='
            f'pam gateway list')
        step += 1

        # Check if there are KSM or missing credential issues
        has_cred_issues = bool(ksm_unresolved) or any(
            'missing password' in w for w in (warnings or []))
        if has_cred_issues:
            cred_note = 'Open records in the "Needs KSM Credentials" subfolder and add passwords.'
            if not ksm_unresolved:
                cred_note = 'Some records were imported without passwords — add credentials in vault.'
            fields.append(
                f'c.text.Step {step}: Add missing credentials={cred_note}')
            step += 1

        fields.append(
            f'c.text.Step {step}: Test a connection='
            f'Open a PAM connection in the vault to verify it works.')
        step += 1

        if project_name:
            fields.append(
                f'c.text.Step {step}: Clean up when done testing='
                f'pam project kcm-cleanup --name "{project_name}" --dry-run')
            step += 1

        # --- Asset reference fields ---
        if gw_name:
            fields.append(f'c.text.Gateway Name={gw_name}')
        if gw_uid:
            fields.append(f'c.text.Gateway UID={gw_uid}')
        if app_uid:
            fields.append(f'c.text.KSM App UID={app_uid}')
        if res_sf:
            fields.append(f'c.text.Resources Folder={res_sf}')
        if res_sf_uid:
            fields.append(f'c.text.Resources Folder UID={res_sf_uid}')
        if usr_sf:
            fields.append(f'c.text.Users Folder={usr_sf}')
        if usr_sf_uid:
            fields.append(f'c.text.Users Folder UID={usr_sf_uid}')
        ...
        try:
            ...
            if fields:
                args['fields'] = fields
            uid = RecordAddCommand().execute(params, **args)
            if not uid:
            ...
            try:
                ...
                if record:
                    tmp_fd, tmp_path = tempfile.mkstemp(
                        suffix='.md', prefix='KCM-Import-Report-')
                    try:
                        with os.fdopen(tmp_fd, 'w') as f:
                            f.write(notes_text)
                        upload_task = att.FileUploadTask(tmp_path)
                        upload_task.name = 'KCM-Import-Report.md'
                ...
                logging.debug('Could not attach report file: %s', e)
    ...
    def _create_project_skeleton(params, project_name, pam_json):
        ...
        PAM config) without records.

        Returns (config_uid, gateway_token) where gateway_token is the
        access_token printed by edit.py during gateway creation, or ''
        ...
        try:
            ...
            # Capture stdout to extract the gateway access_token
            # edit.py prints JSON with {"access_token": "..."} at the end
            stdout_trap = io.StringIO()
            with _STDOUT_LOCK:
                original_stdout = sys.stdout
                sys.stdout = stdout_trap
                try:
                    ...
                    sys.stdout = original_stdout
                    captured_output = stdout_trap.getvalue()
        ...
        # Extract gateway token from captured JSON output
        gateway_token = ''
        try:
            # edit.py prints a JSON blob with "access_token" key
            ...
            for line in captured_output.splitlines():
                ...
                if stripped.startswith('{'):
                    try:
                        ...
                        continue
            # If single-line parse failed, try parsing the full output
            # (edit.py uses json.dumps with indent=2, so it's multi-line)
            ...
            logging.debug('Could not parse gateway token from output: %s', e)
        ...
        for cfg in vault_extensions.find_records(params, record_version=6):
            if cfg.title == config_name:
                return cfg.record_uid, gateway_token
    ...
    def _resolve_db_password(self, params, kwargs):
        record_ref = kwargs.get('db_password_record') or ''
        if record_ref:
            pw = self._extract_password_from_record(params, record_ref)
            if pw:
                return pw
            raise CommandError('kcm-import',
                f'Record "{record_ref}" not found or has no password field')

        # Interactive: search vault for candidate records
        if not getattr(params, 'batch_mode', False):
            candidates = self._search_vault_for_db_records(params)
            if candidates:
                print('\nDatabase Password')
                print('─' * 50)
                print('  Found vault records that may contain the DB password:\n')
                ...
                if choice and choice.upper() != 'M':
                    try:
                        ...
                        if 0 <= idx < len(candidates):
                            ...
                            pw = self._extract_password_from_record(
                                params, rec.record_uid)
                            if pw:
                                return pw
                            logging.warning('Selected record has no password.')
                    except (ValueError, IndexError):
    ...
    @staticmethod
    def _extract_password_from_record(params, record_ref):
        """Load a vault record by UID or title and extract its password."""
        record = vault.KeeperRecord.load(params, record_ref)
        if not record:
            return None
    ...
    @staticmethod
    def _search_vault_for_db_records(params):
        """Search vault for records that might contain a DB password."""
        search_terms = ['guacamole', 'kcm', 'database password', 'db password']
        candidates = []
        seen_uids = set()
        for uid in params.record_cache:
            rec = vault.KeeperRecord.load(params, uid)
            if not rec or uid in seen_uids:
                continue
            title = rec.title if hasattr(rec, 'title') else ''
            title_lower = title.lower()
            if not any(t in title_lower for t in search_terms):
                continue
            # Check if record has a password field
            has_pw = False
            if hasattr(rec, 'password') and rec.password:
                has_pw = True
            elif hasattr(rec, 'get_typed_field'):
                field = rec.get_typed_field('password')
                if field and field.value:
                    has_pw = True
            if has_pw:
                seen_uids.add(uid)
    ...
    @staticmethod
    def _discover_docker_container():
        """Scan running Docker containers for one with Guacamole DB env vars.

        Returns the container name, or raises CommandError if none found.
        """
        try:
            ...
            raise CommandError('kcm-import',
                f'Docker not available: {type(e).__name__}. '
                f'Use --docker-container to specify manually.')

        if result.returncode != 0:
            raise CommandError('kcm-import',
                'Could not list Docker containers. Is Docker running?')

        containers = [c.strip() for c in result.stdout.strip().splitlines()
                      if c.strip()]
        if not containers:
            raise CommandError('kcm-import',
        ...
        db_env_vars = {
            'GUACAMOLE_PASSWORD', 'MYSQL_PASSWORD',
        ...
        for name in containers:
            ...
            if result.returncode != 0:
                continue
            env_keys = {line.split('=', 1)[0]
                        for line in result.stdout.strip().splitlines()
                        if '=' in line}
            result = None  # clear raw output
            if env_keys & db_env_vars:
                candidates.append(name)

        if not candidates:
            raise CommandError('kcm-import',
                'No running containers with Guacamole/KCM database credentials '
                'found. Use --docker-container to specify the container name.')

        if len(candidates) == 1:
            logging.info('Auto-discovered Docker container: %s', candidates[0])
            return candidates[0]

        # Multiple candidates — apply tiered filtering to find the KCM DB
        # Tier 1: KCM-specific containers (name contains 'kcm' and 'db')
        kcm_db = [c for c in candidates
                  if 'kcm' in c.lower() and 'db' in c.lower()]
        if len(kcm_db) == 1:
            logging.info('Auto-discovered KCM DB container: %s '
                         '(selected from %d candidates)', kcm_db[0], len(candidates))
            return kcm_db[0]
        # Tier 2: container with 'guacamole' AND 'db' in name
        guac_db = [c for c in candidates
                   if 'guacamole' in c.lower() and 'db' in c.lower()]
        if len(guac_db) == 1:
            logging.info('Auto-discovered Docker container: %s '
                         '(selected from %d candidates)', guac_db[0], len(candidates))
            return guac_db[0]
        # Tier 3: container with 'guacamole' in name
        guac = [c for c in candidates if 'guacamole' in c.lower()]
        if len(guac) == 1:
            logging.info('Auto-discovered Docker container: %s '
                         '(selected from %d candidates)', guac[0], len(candidates))
            return guac[0]

        # Interactive selection when multiple candidates remain
        print('\nMultiple Docker containers with database credentials found:\n')
        for i, name in enumerate(candidates, 1):
            print(f'  [{i}] {name}')
        print()
        try:
            choice = input(f'  Select container [1-{len(candidates)}]: ').strip()
            idx = int(choice) - 1
            if 0 <= idx < len(candidates):
                logging.info('User selected Docker container: %s',
                             candidates[idx])
        ...
        raise CommandError('kcm-import',
            f'Could not auto-select container. Found {len(candidates)}: '
            f'{names_str}. Use --docker-container to specify one.')

    @staticmethod
    def _detect_db_type_from_docker(container):
        """Detect database type from Docker container environment variables.

        Returns 'mysql' or 'postgresql'.
        """
        try:
            result = subprocess.run(
                ['docker', 'inspect', '--format',
        ...
        has_pg = any(k.startswith(('POSTGRES_', 'POSTGRESQL_'))
                     for k in env_keys)
        has_mysql = any(k.startswith('MYSQL_') for k in env_keys)
        ...
        if has_pg and has_mysql:
            return 'postgresql'  # KCM standard

        # Check GUACAMOLE_* hints
        for k in env_keys:
            if 'POSTGRES' in k:
                return 'postgresql'
            if 'MYSQL' in k:
    ...
    def _detect_docker_credentials(db_type, container='guacamole'):
        ...
        try:
            ...
            # When the container IS the database (no hostname env var),
            # get its own IP directly from Docker networking
            if not host or host in ('127.0.0.1', 'localhost'):
                container_ip = PAMProjectKCMImportCommand._get_container_ip(
                    container)
                if container_ip:
                    host = container_ip
                    logging.info('Using container IP: %s (from %s)',
                                 host, container)
                else:
            ...
            if host:
                host = PAMProjectKCMImportCommand._resolve_docker_host(
                    host, container)
    ...
    def _get_container_ip(container):
        """Get a Docker container's IP address from its network settings.

        Returns the first non-empty IP, or '' if unavailable.
        """
        try:
            result = subprocess.run(
                ['docker', 'inspect', '--format',
...
Examples:
  ...
  pam project kcm-cleanup --config VxANFEPLi8E9gdtlDmfBvw --yes
    ...
    def execute(self, params, **kwargs):
        ...
        # Step 5: Count records in shared folders (only KCM-imported records)
        record_uids = set()
        kcm_prefixes = ('KCM Resource - ', 'KCM User - ')
        for sf_uid in sf_uids:
            sf = params.shared_folder_cache.get(sf_uid, {})
            for rec in sf.get('records', []):
                rec_uid = rec.get('record_uid', '')
                if rec_uid:
                    rec_obj = vault.KeeperRecord.load(params, rec_uid)
                    title = getattr(rec_obj, 'title', '') if rec_obj else ''
                    if title.startswith(kcm_prefixes):
