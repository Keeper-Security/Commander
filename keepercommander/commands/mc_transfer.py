...
class McTransferMixin:
    ...
    def transfer_status_to_text(status: MCTransfer_pb2.MCTransferStatus) -> str:
        ...
        if status == MCTransfer_pb2.MCTransferStatus.STATUS_DENIED:
            return 'Denied'
        if status == MCTransfer_pb2.MCTransferStatus.STATUS_READY:
            return 'Ready'
        return 'Unsupported'

    @staticmethod
    def get_transfer_parameters(command_name: str, **kwargs) -> Tuple[str, str]:
        enterprise_name = kwargs.get('target_name')    # type: Optional[str]
        if not enterprise_name:
            raise error.CommandError(command_name, 'Target enterprise name cannot be empty')

        enterprise_email = kwargs.get('target_email')   # type: Optional[str]
    ...
    def selected_managed_companies(params) -> Set[int]:
        ...
        if isinstance(managed_companies, list) and managed_companies:
            ...
            for mc in managed_companies:
                seats = mc.get('number_of_seats', 0)
                if seats > 2000000:
                    seats = '*'
                ...
                if answer in ['a', 'all']:
                    ...
                    for mc_id in mc_ids:
                        try:
                            ...
                            logging.info(f'"{mc_id}" is not a valid Managed Company ID: {e}')
...
class McTransferPerformCommand(enterprise_common.EnterpriseCommand, McTransferMixin):
    ...
    def execute(self, params, **kwargs):
        ...
        transfer = api.communicate_rest(params, rq, 'enterprise/mc_transfer_status', rs_type=MCTransfer_pb2.MCTransferState)
        if not transfer:
            raise error.CommandError('mc-transfer perform', 'Response cannot be empty')
        ...
        if transfer.receivingEnterpriseName:
            ...
            if len(transfer.mcTransferEnterprises) > 0:
                managed_companies = params.enterprise.get('managed_companies') or []
                has_failed_mc = False
                        ...
                        if isinstance(mc, dict):
                            ...
                            if enterprise_tree_key:
                                try:
                                    ...
                                    logging.info(f'"{id_mc}" decrypt key error: {e}')
                                    has_failed_mc = True
                if has_failed_mc:
                    transfer_self = False
            ...
            if transfer_self:
                try:
                    ...
                    logging.warning(f'Failed to encrypt enterprise key: ID: {transfer.movingEnterpriseId}, Name: {transfer.movingEnterpriseName}, Error: {e}')
