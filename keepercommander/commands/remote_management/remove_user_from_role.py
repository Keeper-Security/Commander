...
class GatewayActionRmRemoveUserFromRoleCommandInputs:
...
class GatewayActionRmRemoveUserFromRoleCommand(GatewayAction):

    def __init__(self, inputs: GatewayActionRmRemoveUserFromRoleCommandInputs, conversation_id=None):
        super().__init__('rm-remove-user-from-role', inputs=inputs, conversation_id=conversation_id,
                         is_scheduled=True)
...
class RmRemoveUserFromRoleCommand(PAMGatewayActionDiscoverCommandBase):
    parser = argparse.ArgumentParser(prog='pam-rm-role-remove')
    ...
    def get_parser(self):
        return RmRemoveUserFromRoleCommand.parser
    ...
    def execute(self, params: KeeperParams, **kwargs):
        ...
        action_inputs = GatewayActionRmRemoveUserFromRoleCommandInputs(
        ...
        router_response = router_send_action_to_gateway(
            params=params,
            gateway_action=GatewayActionRmRemoveUserFromRoleCommand(
        ...
        if data is None:
            ...
            print(f"{bcolors.FAIL}Could not remove user from role: {error}{bcolors.ENDC}")
            return

        print("")
        print(f"{bcolors.OKGREEN}User removed from role.{bcolors.ENDC}")
        print("")
