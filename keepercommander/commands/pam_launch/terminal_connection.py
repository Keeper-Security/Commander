...
def _notify_gateway_connection_close(params, gateway_cookies, router_token, terminated=True):
    """
    ...
    if not router_token:
        ...
        return
    if not gateway_cookies:
        logging.debug("Skipping connection_close notification - gateway cookies missing")
        return
    ...
    try:
        ...
        if response.status_code >= 400:
            ...
            logging.info("Sent connection_close notification for router token")
...
def _open_terminal_webrtc_tunnel(params: KeeperParams,
    ...
    """
    logging.info(f"{bcolors.HIGHINTENSITYWHITE}Establishing {protocol.upper()} terminal connection via WebRTC...{bcolors.ENDC}")
    ...
    try:
        ...
        tunnel_session.router_token = router_token
        ...
        # Start WebSocket listener with cookies for session affinity
        ...
        try:
            ...
            # Add router token if available
            if gateway_cookies:
                # Extract router token from cookies
                import re
                for cookie_name, cookie_value in gateway_cookies.items():
                    if 'router' in cookie_name.lower() or 'token' in cookie_name.lower():
                        inputs['routerToken'] = cookie_value
                        router_token = cookie_value
                        break
            ...
            if trickle_ice:
                ...
                router_response = router_send_action_to_gateway(
                    ...
                    destination_gateway_cookies=gateway_cookies
                )
            ...
            unregister_conversation_key(conversation_id)
            _notify_gateway_connection_close(params, gateway_cookies, router_token)
