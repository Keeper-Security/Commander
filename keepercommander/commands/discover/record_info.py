...
class PAMGatewayActionDiscoverRecordInfoCommand(PAMGatewayActionDiscoverCommandBase):
    parser = argparse.ArgumentParser(prog='dr-discover-command-record')
    ...
    def get_parser(self):
        return PAMGatewayActionDiscoverRecordInfoCommand.parser
    ...
    def _show_info(params: KeeperParams, configuration_record: TypedRecord, record: TypedRecord):
        ...
        print("")
        print(f"{bcolors.BOLD}Configuration UID: {configuration_record.record_uid}{bcolors.ENDC}")
        print(f"{bcolors.BOLD}Configuration Key Bytes Hex: {configuration_record.record_key.hex()}{bcolors.ENDC}")
        print("")
        try:
