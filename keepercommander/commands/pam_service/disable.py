...
class PAMActionServiceDisableCommand(PAMGatewayActionDiscoverCommandBase):
    parser = argparse.ArgumentParser(prog='pam action service disable')
    ...
    def get_parser(self):
        return PAMActionServiceDisableCommand.parser

    def execute(self, params: KeeperParams, **kwargs):

        gateway = kwargs.get("gateway", "none_set")
        machine_uid = kwargs.get("machine_uid")  # type: Optional[str]
        ...
        if machine_uid is not None:
            ...
            if (machine_vertex.get_edge(root_vertex, edge_type=EdgeType.LINK) is None and
                    machine_vertex.get_edge(root_vertex, edge_type=EdgeType.ACL) is None):
            ...
            if machine_data is None:
                print(self._f("Could not get the machine's data."))
                return

            if not machine_data.no_update_services:
                machine_data.no_update_services = True
                machine_vertex.add_data(machine_data.model_dump_json(), needs_encryption=False, path="meta")
                record_link.save()

            print(
                self._gr(
                    "Success: Machine will NOT allow services password to be changed during rotation."
                )
            )
            ...
            if user_data is None:
                print(self._f("Could not get the user's data."))
                return

            if not user_data.no_update_services:
                user_data.no_update_services = True
                user_vertex.add_data(user_data.model_dump_json(), needs_encryption=False, path="meta")
                record_link.save()

            print(
                self._gr(
                    "Success: When rotating the user's password, service passwords will NOT be changed, if applicable."
                )
            )
