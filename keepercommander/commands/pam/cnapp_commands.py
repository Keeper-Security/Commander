...
def _load_encrypter_key(params, config_record_uid):
    ...
    try:
        ...
        logging.debug('CNAPP: failed to load encrypter record %s: %s', config_record_uid, e)
        return None
...
class PAMCnappConfigReadCommand(Command):
    ...
    def execute(self, params, **kwargs):
        ...
        if response is None:
            logging.warning('No CNAPP configuration returned.')
            return None
    ...
    def _resolve_encrypter_key(self, params, kwargs):
        ...
        if not config_record_uid:
            try:
                ...
                logging.debug('CNAPP: could not read configuration for decryption: %s', e)
                return None, None
