...
```bash
docker run -d -p 9007:9007 \
  -v /Users/sharad-keeper/Downloads/ksm-config:/home/commander/ksm-config.json \
  keeper-commander \
  service-create -p 9007 -c 'ls,tree' -f json \
  --ksm-config /home/commander/ksm-config.json \
  --record ABC123-DEF456-GHI789
...
- **Multiple authentication methods**: Supports credential, config file, KSM config, and KSM token authentication
- **Secure config retrieval**: KSM methods download configs at runtime without exposing sensitive data
- **Centralized secret management**: KSM integration provides centralized control over service configurations
- **Audit logging**: All KSM-based config access is tracked in Keeper audit logs
