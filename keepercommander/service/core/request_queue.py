...
from ..config.models import DEFAULT_QUEUE_MAX_SIZE, DEFAULT_REQUEST_TIMEOUT, DEFAULT_RESULT_RETENTION
...
class RequestQueueManager:
    ...
    def __init__(self):
        ...
        # Initialize with default values - will be updated from config when available
        ...
        logger.info("RequestQueueManager initialized")
    ...
    def _load_config(self):
        """Load configuration settings from service config."""
        try:
            ...
            max_size = config_data.get('queue_max_size', DEFAULT_QUEUE_MAX_SIZE)
            self.request_timeout = config_data.get('request_timeout', DEFAULT_REQUEST_TIMEOUT)
            self.result_retention = config_data.get('result_retention', DEFAULT_RESULT_RETENTION)
            
            # Recreate queue with new max size if different
            if self.request_queue.maxsize != max_size:
                self.request_queue = queue.Queue(maxsize=max_size)
                
            self._config_loaded = True
            logger.info(f"Queue configuration loaded: max_size={max_size}, "
                       f"timeout={self.request_timeout}s, retention={self.result_retention}s")
            
        except Exception as e:
            logger.warning(f"Could not load queue configuration, using defaults: {e}")
            self._config_loaded = True
