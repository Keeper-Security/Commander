...
parser.add_argument('-p', '--manage-records', dest='manage_records', action='store_true',
                    help='account permission: can manage records.')
parser.add_argument('-o', '--manage-users', dest='manage_users', action='store_true',
...
existing_users = set()
if isinstance(sf_rs.get('users'), list):
    existing_users.update((x.get('email').lower() for x in sf_rs['users']))
    ...
    if isinstance(opts.add, list):
        for email in opts.add:
            ...
            if email in existing_users:
                logging.info('Add user "%s": already belongs to the shared folder')
            else:
    ...
    if isinstance(opts.remove, list):
        for email in opts.remove:
            ...
            if email in existing_users:
                ...
                logging.info('Remove user "%s": does not belongs to the shared folder')

if len(users_to_add) > 0:
    public_keys = {x: None for x in users_to_add}
    api.load_user_public_keys(my_params, users_to_add, False)

manage_users = opts.manage_users
manage_records = opts.manage_records
...
for add_status in rs.sharedFolderAddUserStatus:
    if add_status.status != 'success':
        logging.info(f'Failed to add user {add_status.username} to shared folder: {add_status.status}')
for remove_status in rs.sharedFolderRemoveUserStatus:
    if remove_status.status != 'success':
        logging.info(f'Failed to add user {remove_status.username} to shared folder: {remove_status.status}')
