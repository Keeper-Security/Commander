    ...
    def endElement(self, name):
        ...
        if len(self.stack) == 2:
            if self.stack[1] == 'SecretTemplates':
                if name == 'secrettype':
                    if self.template is not None:
                        self.templates[self.template['name']] = self.template
                        self.template = None
                    else:
            ...
            if self.template is not None:
                ...
                if self.stack[3] == 'fields':
                    if len(self.stack) == 6 and self.stack[4] == 'field':
                        if self.stack[5] == 'name':
                            self.template['field_name'] = content
                        elif content == 'true' and self.template['field_name']:
                            if self.stack[5] in ('isurl', 'ispassword', 'isnotes', 'isfile'):
                                self.template['fields'][self.template['field_name']] = self.stack[5]
            ...
            if self.secret is not None:
                if len(self.stack) == 4:
                    ...
                    if self.stack[5] == 'FieldName':
                        self.secret['last_name'] = content
                    elif self.stack[5] == 'Value':
                        if self.secret['last_name']:
                            if content:
                                self.secret['items'][self.secret['last_name']] = content
                            self.secret['last_name'] = ''
                        else:
                            logging.info('characters: FieldName has not been set')
            else:
...
for secret in handler.secrets:
    ...
    for login_field in ('username', 'client-id'):
        if has_field(items, login_field):
            username = pop_field_value(items, login_field)
            if username:
                if record.login:
                    record.fields.append(RecordField(type='login', label='', value=username))
                else:
                    record.login = username

    for password_field in ('password', 'client-secret'):
        if has_field(items, password_field):
            password = pop_field_value(items, password_field)
            if password:
                if record.password:
                    record.fields.append(RecordField(type='password', label='', value=password))
                else:
    ...
    for phone_slug in ('contact-number', 'work-phone', 'home-phone', 'mobile-phone', 'fax'):
        phone_number = pop_field_value(items, phone_slug)
        if phone_number:
            ...
            if phone:
                if phone_slug.startswith('work'):
                    ...
                    phone['type'] = 'Home'
                record.fields.append(RecordField(type='phone', label='', value=phone))

    for url_slug in ('website', 'blog', 'resource', 'url', 'tenant'):
        url = pop_field_value(items, url_slug)
        if url:
            if record.login_url:
                record.fields.append(RecordField(type='url', label=url_slug, value=url))
            else:
    ...
    for num_slug in ('policy-number', 'group-number'):
        if has_field(items, num_slug):
            number = pop_field_value(items, num_slug)
            if number:
                record.fields.append(RecordField(type='accountNumber', label=num_slug, value=number))
    ...
    for slug in list(items.keys()):
        field_value = pop_field_value(items, slug)
        if not field_value:
            continue
        slug_type = ''
        ...
        if slug.lower() == 'totp key':
            field_value = f'otpauth://totp/?secret={field_value}'
