...
class TestEnterprise(TestCase):
    ...
    def setUp(self):
        ...
        self.public_keys_mock = mock.patch('keepercommander.api.load_user_public_keys').start()
        self.public_keys_mock.side_effect = TestEnterprise.load_user_public_keys
...
[
    ...
    def load_user_public_keys(params, emails, send_invites=False):
        ...
        for email in s:
            rsa_key = crypto.unload_rsa_public_key(vault_env.public_key)
            ec_key = crypto.unload_ec_public_key(vault_env.ec_public_key)
            params.key_cache[email.casefold()] = PublicKeys(rsa=rsa_key, ec=ec_key)
