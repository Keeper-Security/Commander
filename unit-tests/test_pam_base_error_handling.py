...
class TestCronFieldExceptionSplit(unittest.TestCase):
    """import_schedule_field should split JSONDecodeError from ValueError/KeyError."""

    def test_cron_invalid_json(self):
        """Invalid JSON in CRON field should log error."""
        data = {"type": "cron", "cron": "{not-json"}
        with self.assertLogs(level='ERROR') as cm:
            PamRotationScheduleObject.load(json.dumps(data))
        self.assertTrue(
            any('CRON' in msg or 'invalid JSON' in msg for msg in cm.output)
        )
    ...
    def test_type_builtin_not_shadowed(self):
        ...
        """
        data = {"type": "cron", "cron": '{"time": "invalid"}'}
        # Must not raise TypeError from shadowed type() call
        obj = PamRotationScheduleObject.load(json.dumps(data))
