...
class TestSiemExport(unittest.TestCase):
    ...
    def test_ndjson_output_format(self):
        ...
        result = SecurityAuditReportCommand._export_siem(
            ...
            show_breachwatch=False, filename=None, title='Test'
        )
    ...
    def test_breachwatch_mode(self):
        ...
        result = SecurityAuditReportCommand._export_siem(
            ...
            show_breachwatch=True, filename=None, title='Test'
        )
    ...
    def test_file_output_permissions(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            SecurityAuditReportCommand._export_siem(
                self._make_params(), self._sample_rows(), self._fields(),
                show_breachwatch=False, filename=filepath, title='Test'
            )
    ...
    def test_source_field_from_params(self):
        ...
        result = SecurityAuditReportCommand._export_siem(
            ...
            self._fields(), show_breachwatch=False, filename=None, title='Test'
        )
        event = json.loads(result.strip().split('\n')[0])
...
class TestSiemFlagValidation(unittest.TestCase):
    ...
    def test_siem_with_record_details_raises(self):
        ...
        # We test the validation logic directly by checking the code path.
        # The actual execute() requires full enterprise params, but the
        # validation is at the top of the method after save_report.
        # We verify via source inspection that the check exists.
        import inspect
        source = inspect.getsource(SecurityAuditReportCommand.execute)
        self.assertIn("--siem and --record-details cannot be used together", source)

    def test_siem_with_format_raises(self):
        """--siem and --format should be rejected."""
        import inspect
        source = inspect.getsource(SecurityAuditReportCommand.execute)
        self.assertIn("--siem produces NDJSON output exclusively", source)
