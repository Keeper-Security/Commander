...
if sys.version_info >= (3, 8):
    ...
    from keepercommander.service.decorators.security import security_check, is_blocked_ip
    ...
    class TestAuthSecurity(TestCase):
        ...
        def test_is_blocked_ip_single_ip(self):
            ...
            self.assertTrue(is_blocked_ip("192.168.1.1", blocked_ips))
            self.assertFalse(is_blocked_ip("192.168.1.2", blocked_ips))

        def test_is_blocked_ip_cidr(self):
            """Test IP blocking with CIDR notation"""
            blocked_ips = "192.168.1.0/24"
            self.assertTrue(is_blocked_ip("192.168.1.1", blocked_ips))
            self.assertTrue(is_blocked_ip("192.168.1.254", blocked_ips))
            self.assertFalse(is_blocked_ip("192.168.2.1", blocked_ips))
