...
from keepercommander.commands.discoveryrotation import PAMCreateRecordRotationCommand, PAMResourceRotateCommand, \
    PAMListRecordRotationCommand, PAMGatewayListCommand
...
class TestPAMResourceRotateCommand(unittest.TestCase):
    ...
    def setUp(self):
        self.command = PAMResourceRotateCommand()
        self.parser = self.command.get_parser()

    def test_parser(self):
        args = self.parser.parse_args(['uid', '--enable'])
        self.assertEqual(args.uid, 'uid')
        self.assertTrue(args.enable)

    @patch('keepercommander.vault.KeeperRecord.load')
    @patch('keepercommander.commands.discoveryrotation.get_keeper_tokens', return_value=(b'token', b'encrypted_key', b'transmission_key'))
    @patch('keepercommander.commands.discoveryrotation.TunnelDAG')
    def test_execute_with_enable(self, mock_TunnelDAG, mock_get_keeper_tokens, mock_load):
    ...
    @patch('keepercommander.commands.discoveryrotation.get_keeper_tokens', return_value=(b'token', b'encrypted_key', b'transmission_key'))
    @patch('keepercommander.commands.discoveryrotation.TunnelDAG')
    def test_execute_with_disable(self, mock_TunnelDAG, mock_get_keeper_tokens, mock_load):
        mock_params, mock_typed_record = create_mock_params_and_record('pamMachine')
        mock_load.return_value = mock_typed_record
    ...
    @patch('keepercommander.commands.discoveryrotation.get_keeper_tokens', return_value=(b'token', b'encrypted_key', b'transmission_key'))
    @patch('keepercommander.commands.discoveryrotation.TunnelDAG')
    def test_execute_with_enable_and_admin(self, mock_TunnelDAG, mock_get_keeper_tokens, mock_load):
        mock_params, mock_typed_record = create_mock_params_and_record('pamMachine')
        mock_load.return_value = mock_typed_record
