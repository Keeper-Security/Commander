...
class TestKeyDecode(unittest.TestCase):
    ...
    decoded length is exactly 32 bytes (AES-256). 16-byte keys are rejected."""
    ...
    def test_16_bytes_returns_none(self):
        raw = base64.b64encode(b'\x44' * 16).decode('ascii')
...
class TestLoadEncrypterKey(unittest.TestCase):
    """_load_encrypter_key must try all labeled candidates before giving up."""

    VALID_KEY = b'\xAB' * 32
    INVALID_RAW = 'not-a-valid-key!!'

    def _make_typed_field(self, type_ref, label=None, value=None):
        field = MagicMock()
        field.type = type_ref
        field.label = label
    ...
    def _make_record(self, secret_labeled=None, note_labeled=None, note_unlabeled=None):
        record = MagicMock(spec=cnapp_commands.vault.TypedRecord)

        def get_typed_field(type_ref, label=None):
            if type_ref == 'secret' and label == cnapp_commands.CNAPP_ENCRYPTION_KEY_LABEL:
                return secret_labeled
            if type_ref == 'note' and label == cnapp_commands.CNAPP_ENCRYPTION_KEY_LABEL:
                return note_labeled
            if type_ref == 'note' and label is None:
                return note_unlabeled
            return None

        record.get_typed_field.side_effect = get_typed_field
        return record

    def _field_with_value(self, raw):
        f = MagicMock()
        f.value = [raw]
        return f

    def test_returns_valid_secret_labeled_key(self):
        raw = base64.b64encode(self.VALID_KEY).decode('ascii')
        record = self._make_record(secret_labeled=self._field_with_value(raw))
        params = MagicMock()
        with patch.object(cnapp_commands.vault.KeeperRecord, 'load', return_value=record):
            result = cnapp_commands._load_encrypter_key(params, 'uid123')
        self.assertEqual(result, self.VALID_KEY)

    def test_falls_through_to_note_labeled_when_secret_invalid(self):
        """When the secret-labeled field is invalid, note-labeled field must still be tried."""
        valid_raw = base64.b64encode(self.VALID_KEY).decode('ascii')
        record = self._make_record(
            secret_labeled=self._field_with_value(self.INVALID_RAW),
            note_labeled=self._field_with_value(valid_raw),
        )
        params = MagicMock()
        with patch.object(cnapp_commands.vault.KeeperRecord, 'load', return_value=record):
            result = cnapp_commands._load_encrypter_key(params, 'uid123')
    ...
    def test_warns_and_returns_none_when_all_labeled_invalid(self):
        """If labeled fields exist but all are invalid, warn and return None (no unlabeled fallback)."""
        unlabeled = self._field_with_value(base64.b64encode(self.VALID_KEY).decode('ascii'))
        record = self._make_record(
            secret_labeled=self._field_with_value(self.INVALID_RAW),
            note_labeled=self._field_with_value(self.INVALID_RAW),
            note_unlabeled=unlabeled,
        )
        params = MagicMock()
        with patch.object(cnapp_commands.vault.KeeperRecord, 'load', return_value=record):
            with self.assertLogs(cnapp_commands.__name__, level='WARNING') as cm:
                result = cnapp_commands._load_encrypter_key(params, 'uid123')
        self.assertIsNone(result)
        self.assertTrue(any('not a valid AES-256 key' in msg for msg in cm.output))

    def test_falls_back_to_unlabeled_note_when_no_labeled_fields(self):
        """When no labeled key fields exist at all, the first unlabeled note field is used."""
        valid_raw = base64.b64encode(self.VALID_KEY).decode('ascii')
        unlabeled = self._field_with_value(valid_raw)
        record = self._make_record(note_unlabeled=unlabeled)
        params = MagicMock()
        with patch.object(cnapp_commands.vault.KeeperRecord, 'load', return_value=record):
            result = cnapp_commands._load_encrypter_key(params, 'uid123')
        self.assertEqual(result, self.VALID_KEY)

    def test_returns_none_for_missing_record_uid(self):
        params = MagicMock()
        self.assertIsNone(cnapp_commands._load_encrypter_key(params, None))
        self.assertIsNone(cnapp_commands._load_encrypter_key(params, ''))
